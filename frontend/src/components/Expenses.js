...
const Expenses = () => {
  ...
  const [supplierAbonos, setSupplierAbonos] = useState({}); // Abonos de cada supplierExpense: { [expenseId]: [abonos] }
                              ...
                              return (
                                <>
                                  ...
                                  {(() => {
                                    ...
                                    // Buscar servicios directamente de la reservación asociada
                                    let servicesCount = 0;
                                    if (expense.related_reservation_id) {
                                      const reservation = reservations.find(r => r.id === expense.related_reservation_id);
                                      if (reservation && reservation.extra_services) {
                                        servicesCount = reservation.extra_services.length;
                                      }
                                    }
                                    
                                    // Si no hay reservación, intentar detectar del texto (para gastos viejos)
                                    if (servicesCount === 0) {
                                      const servicesMatch = description.match(/Incluye (\d+) servicio\(s\) adicional\(es\)/);
                                      servicesCount = servicesMatch ? parseInt(servicesMatch[1]) : 0;
                                    }
                                    
                                    // Detectar si incluye extras (horas o personas)
                                    const hasExtras = description.includes('Horas extras') || description.includes('Personas extras');
                                    const hasITBIS = description.includes('Con ITBIS');
                                    const isAutoGenerated = description.includes('Auto-generado');
                                          ...
                                          <div className="flex items-center gap-2 flex-wrap">
                                            ...
                                            {servicesCount > 0 && (
                                              <span className="text-xs px-3 py-1.5 bg-purple-200 text-purple-900 rounded-full font-bold border-2 border-purple-400 shadow-sm" title={`Incluye ${servicesCount} servicio(s) adicional(es)`}>
                                                🛎️ {servicesCount} Servicio{servicesCount > 1 ? 's' : ''}
                                              </span>
                                            )}
                                            {/* Badge para extras */}
                                            {hasExtras && (
                                              <span className="text-xs px-2 py-1 bg-orange-100 text-orange-700 rounded-full font-medium" title="Incluye horas extras o personas extras">
                                                ➕ Extras
                                              </span>
                                            )}
                                            {hasITBIS && (
                                              <span className="text-xs px-2 py-0.5 bg-blue-100 text-blue-700 rounded-full font-medium" title="Con ITBIS">
                                                📋 ITBIS
                                              </span>
                                            )}
                                            {isAutoGenerated && (
                                              <span className="text-xs px-2 py-0.5 bg-green-100 text-green-700 rounded-full font-medium" title="Auto-generado por reservación">
                                                ⚙️ Auto
                                              </span>
                                            )}
                                          </div>
                                          {/* Hint de click */}
                                          {(description.split('\n').length > 1 || servicesCount > 0 || hasExtras) && (
                                            <div className="text-xs text-blue-600 italic font-medium">
                                              👆 Click para ver detalles {servicesCount > 0 ? 'y servicios' : ''}
                                            </div>
                                          )}
                                        </div>
                      ...
                      {relatedReservation.extra_services.map((service, index) => {
                        ...
                        return service.supplier_name && (
                          <div key={index} className="p-4 bg-orange-50 border border-orange-200 rounded-md">
                            ...
                            {(() => {
                              // Buscar abonos por supplier_name directamente en supplierAbonos
                              let abonosDelServicio = [];
                              
                              // Si tenemos supplierExpense, usarlo
                              if (supplierExpense && supplierAbonos[supplierExpense.id]) {
                                abonosDelServicio = supplierAbonos[supplierExpense.id];
                              } else {
                                // Si no, buscar en todos los supplierAbonos por nombre del suplidor
                                for (const [expenseId, abonos] of Object.entries(supplierAbonos)) {
                                  if (abonos && abonos.length > 0) {
                                    // Buscar en supplierExpenses si alguno coincide con este servicio
                                    const matchingExpense = supplierExpenses.find(e => 
                                      e.id === expenseId && e.description.includes(service.supplier_name)
                                    );
                                    if (matchingExpense) {
                                      abonosDelServicio = abonos;
                                      break;
                                    }
                                  }
                                }
                              ...
                              console.log('🔍 [HISTORIAL] Servicio:', service.supplier_name);
                              console.log('🔍 [HISTORIAL] Abonos encontrados:', abonosDelServicio.length);
                              
                              return abonosDelServicio.length > 0 ? (
                                <div className="mt-4 pt-4 border-t border-orange-300">
                                  <h5 className="text-sm font-semibold text-orange-900 mb-2">
                                    Historial de Pagos ({abonosDelServicio.length})
                                  </h5>
                                  <div className="space-y-2">
                                    {abonosDelServicio.map((abono) => (
                                      <div key={abono.id} className="flex justify-between items-center p-2 bg-white rounded border border-orange-200">
                                        ...
                                        {user?.role === 'admin' && supplierExpense && (
                                          <Button
                                            size="sm"
                                            variant="ghost"
              ...
              {(() => {
                ...
                return (hasExtras || hasServices || hasITBIS) && (
                  <div className="p-4 bg-blue-50 border border-blue-200 rounded-lg">
                    <p className="text-sm text-blue-800 font-bold mb-3">📋 Incluye:</p>
                    <div className="space-y-2">
                      {hasExtras && (
                        <div className="flex items-center space-x-2">
                          <span className="text-xl">➕</span>
                          <span className="font-medium">Extras (horas o personas adicionales)</span>
                        </div>
                      )}
                      {hasServices && (
                        <div className="flex items-center space-x-2">
                      ...
                      {hasITBIS && (
                        <div className="flex items-center space-x-2">
                          <span className="text-xl">📋</span>
                          <span className="font-medium">ITBIS incluido</span>
                        </div>
                      )}
                    </div>
                  </div>
