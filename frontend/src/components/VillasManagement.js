...
const VillasManagementNew = () => {
  ...
  const handleEdit = (villa) => {
    ...
    setFormData({
      ...
      extra_hours_price: villa.extra_hours_price || 0,
      extra_people_price: villa.extra_people_price || 0,
                            ...
                            {flexiblePrices.amanecida.map((price, index) => (
                              <div key={index} className="grid grid-cols-5 gap-2 items-center">
                                <input
                                  type="text"
                                  value={price.people_count || ''}
                                  onChange={(e) => handleUpdateFlexiblePrice('amanecida', index, 'people_count', e.target.value)}
                                  className="px-2 py-1 border rounded text-sm"
                                  placeholder="1-10"
                                />
                                <input
                            ...
                            {flexiblePrices.evento.map((price, index) => (
                              <div key={index} className="grid grid-cols-5 gap-2 items-center">
                                <input
                                  type="text"
                                  value={price.people_count || ''}
                                  onChange={(e) => handleUpdateFlexiblePrice('evento', index, 'people_count', e.target.value)}
                                  className="px-2 py-1 border rounded text-sm"
                                  placeholder="1-10"
                                />
                                <input
                                ...
                                <div className="flex justify-center">
                                  <input
                                    ...
                                    onChange={(e) => {
                                      // Solo permitir 1 checkbox por tipo
                                      const newPrices = [...flexiblePrices.evento];
                                      newPrices.forEach((p, i) => p.is_default = i === index && e.target.checked);
                                      setFlexiblePrices({...flexiblePrices, evento: newPrices});
                                    }}
                                    className="w-4 h-4"
                                  />
                                </div>
